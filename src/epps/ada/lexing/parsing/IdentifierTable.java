...
public class IdentifierTable {
     ...
     * @throws ParsingException - Thrown if the identifier cannot be declared in
    ...
    public void declareIdentifier(IdentifierExpression identifier, Type type) throws ParsingException {
        ...
        // If the identifier is already declared in this scope then we cant declare this
        // identifier, again
    ...
    private boolean doesTypeExist(Type type) {
        return this.knownTypes.stream().filter(type::equals).findAny().isPresent();
    }

    /**
     * Checks to see if there exists any declared identifiers that equal the passed
     * in identifier tith scopes that are included in the current scope.
     * 
    ...
    private boolean identifierDeclaredInScopeAlready(IdentifierExpression identifier) {
        return this.identifierInfos.stream().filter(identifierInfo -> identifierInfo.getIdentifier().equals(identifier))
                .map(IdentifierInformation::getScope).anyMatch(scope -> scope.isIncludedIn(currentScope));
    }
    ...
    /**
     * IdentifierInformation is simply a class to hold various information about an
     * identifier
     */
